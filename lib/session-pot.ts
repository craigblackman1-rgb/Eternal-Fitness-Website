...
export function deriveSessionPot(
  sessions: Pick<DBSession, "status" | "charged_free" | "cancelled_at" | "parent_session_id" | "completed_at" | "data">[],
  ...
  for (const s of potSessions) {
    const status = deriveSessionStatus({ ...s, session_log: s.data?.session_log });
    if (status === "completed") {
      completed++;
