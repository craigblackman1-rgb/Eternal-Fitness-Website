...
- Six more hub screens restyled to match new mockups (dashboard, exercise library, process & quality, reports/updates, SOP detail view, studio equipment) — **code done and type-checked, NOT committed/pushed, NOT visually verified live** — Craig to check `localhost:3001` himself and give the go-ahead. See handoff.md 2026-07-20 (later session) for the two silent OpenCode failures caught during verification and one pre-existing unrelated bug found (`ClientUpdatesPanel.tsx:60`, not fixed).
